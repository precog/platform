...
if status beta-auth-v1 | grep running; then
    stop beta-auth-v1
fi

sleep 5

if ! RESULT=`start beta-auth-v1 2>&1` > /dev/null ; then
...
#curl -v -f -G "http://localhost:30102/blueeyes/services/auth/v1/health"
