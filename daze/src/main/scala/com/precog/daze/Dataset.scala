...
trait GroupingOps[Dataset[_], Valueset[_], Grouping[_, _]] {
  ...
  def flattenGroup[A, K, B](g: Grouping[K, NEL[Dataset[A]]], nextId: () => Identity, memoId: Int, memoCtx: MemoizationContext[Valueset], expiration: Long)(f: (K, NEL[Dataset[A]]) => Dataset[B])(implicit buffering: Buffering[B], fs: IncrementalSerialization[(Identities, B)]): Dataset[B]
