...
trait DiskMemoizationComponent extends YggConfigComponent with MemoizationComponent { 
  ...
  sealed trait MemoContext extends MemoizationContext { self => 
    ...
    def memoizer[X, F[_]](memoId: Int)(implicit MO: F |>=| IO): IterateeT[X, SEvent, F, Either[Vector[SEvent], File]] = {
      import MO._

      def consume(i: Int, acc: Vector[SEvent]): IterateeT[X, SEvent, F, Either[Vector[SEvent], File]] = {
        if (i < yggConfig.memoizationBufferSize) 
          ...
          (writer(new File(yggConfig.memoizationWorkDir, "memo" + memoId)) &= enumStream[X, SEvent, F](acc.toStream)) map (f => Right(f))
      }

      consume(0, Vector.empty[SEvent])
    }
  }
}
