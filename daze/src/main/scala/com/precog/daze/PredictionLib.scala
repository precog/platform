...
trait PredictionLibModule[M[+_]] extends ColumnarTableLibModule[M] with EvaluatorMethodsModule[M] {
  import trans._
  import trans.constants._

  trait PredictionLib extends ColumnarTableLib with EvaluatorMethods {
    ...
    private case class Model(name: String, featureValues: Map[CPath, Double], constant: Double)
    private case class ModelSet(identity: Seq[Option[Long]], models: Set[Model])
    private type Models = List[ModelSet]

    private val reducer: CReducer[Models] = new CReducer[Models] {
    ...
    private def morph1Apply(models: Models, function: Double => Double): Morph1Apply = new Morph1Apply {
