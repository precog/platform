...
trait StringLibSpec[M[+_]] extends Specification
  ...
  def op1Input(op: Op1, loadFrom: Line => dag.LoadLocal) = {
    val line = Line(1, 1, "")
    dag.Operate(BuiltInFunction1Op(op1), loadFrom(line)))(line)
  }

  def op2Input(op: Op2, const: CValue, loadFrom: Line => dag.LoadLocal) = {
    val line = Line(1, 1, "")
    Join(BuiltInFunction2Op(codePointAt), CrossLeftSort, loadFrom(line), Const(const)(line))(line)
  }
  ...
  "for homogeneous sets, the appropriate string function" should {
    ...
    "determine substring with valid integer" in {
      val input = op2Input(substring, CLong(7), homStrings)
    ...
    "determine takeLeft with invalid integer" in {
      val input = op2Input(takeLeft, CLong(7.5), homStrings)
      testEval(input) must haveSize(0)
    }
    "determine takeRight with invalid integer" in {
      val input = op2Input(takeRight, CLong(7.5), homStrings)
      testEval(input) must haveSize(0)
    }
    "determine dropLeft with invalid integer" in {
      val input = op2Input(dropLeft, CLong(7.5), homStrings)
      testEval(input) must haveSize(0)
    }
    "determine dropRight with invalid integer" in {
      val input = op2Input(dropRight, CLong(7.5), homStrings)
      testEval(input) must haveSize(0)
    }
