...
if ! monit unmonitor -g ingest-v1; then
    echo "Monit unhappy on unmonitor of ingest-v1"
fi


# Stop and start the service
if status ingest-v1 | grep running; then
    stop ingest-v1
fi
...
if ! monit monitor -g ingest-v1; then
    echo "Monit unhappy on remonitor of ingest-v1"
fi
