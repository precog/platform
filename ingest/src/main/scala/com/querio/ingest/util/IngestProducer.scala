...
object IngestProducer extends ArbitraryIngestMessage {
  ...
  def main(args: Array[String]) {
    ...
    0.until(messages).foreach { i =>
      ...
      store.save(genRandomEvent.sample.get)
      Thread.sleep(delay)
    }
  }
  ...
  def kafkaStore(topic: String): EventStore = {
    ...
    props.put("serializer.class", "com.querio.ingest.service.IngestMessageCodec")
  ...
  def echoStore(topic: String): EventStore = {
    
    val messageSenderMap = Map() + (MailboxAddress(0L) -> new EchoMessageSender())
