...
object PlatformBuild extends Build {
  ...
  lazy val ragnarok = Project(id = "ragnarok", base = file("ragnarok")).settings(ragnarokSettings ++ commonSettings: _*).dependsOn(muspelheim % "compile->compile;test->test", common % "compile->compile;test->test", quirrel, daze, yggdrasil)
