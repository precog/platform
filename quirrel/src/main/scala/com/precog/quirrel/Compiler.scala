 ...
 */
trait Compiler extends Phases with parser.Parser with typer.TreeShaker with typer.ForallRewriter {
  def compile(str: LineStream): Expr = rewriteForall(shakeTree(parse(str)))
