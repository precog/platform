...
object BinderSpecs extends Specification with ScalaCheck with Parser with StubPhases with Binder {
  ...
  "pre-binding of Time functions" should {
    ...
    "bind weekOfMonth" in {
      val d @ Dispatch(_, _, _) = parse("std::time::weekOfMonth")
      d.binding mustEqual BuiltIn(Identifier(Vector("std", "time"), "weekOfMonth"), 1, false)
      d.isReduction mustEqual false
      d.errors must beEmpty
    }
    ...
    "bind timeZone" in {
      val d @ Dispatch(_, _, _) = parse("std::time::timeZone")
      d.binding mustEqual BuiltIn(Identifier(Vector("std", "time"), "timeZone"), 1, false)
      d.isReduction mustEqual false
      d.errors must beEmpty
    }
    "bind season" in {
      val d @ Dispatch(_, _, _) = parse("std::time::season")
      d.binding mustEqual BuiltIn(Identifier(Vector("std", "time"), "season"), 1, false)
      d.isReduction mustEqual false
      d.errors must beEmpty
    }
    "bind getMillis" in {
      val d @ Dispatch(_, _, _) = parse("std::time::getMillis")
      d.binding mustEqual BuiltIn(Identifier(Vector("std", "time"), "getMillis"), 1, false)
      d.isReduction mustEqual false
      d.errors must beEmpty
    }
    ...
    "bind yearsBetween" in {
      val d @ Dispatch(_, _, _) = parse("std::time::yearsBetween")
      d.binding mustEqual BuiltIn(Identifier(Vector("std", "time"), "yearsBetween"), 2, false)
      d.isReduction mustEqual false
      d.errors must beEmpty
    }
    "bind monthsBetween" in {
      val d @ Dispatch(_, _, _) = parse("std::time::monthsBetween")
      d.binding mustEqual BuiltIn(Identifier(Vector("std", "time"), "monthsBetween"), 2, false)
      d.isReduction mustEqual false
      d.errors must beEmpty
    }
    "bind weeksBetween" in {
      val d @ Dispatch(_, _, _) = parse("std::time::weeksBetween")
      d.binding mustEqual BuiltIn(Identifier(Vector("std", "time"), "weeksBetween"), 2, false)
      d.isReduction mustEqual false
      d.errors must beEmpty
    }
    "bind daysBetween" in {
      val d @ Dispatch(_, _, _) = parse("std::time::daysBetween")
      d.binding mustEqual BuiltIn(Identifier(Vector("std", "time"), "daysBetween"), 2, false)
      d.isReduction mustEqual false
      d.errors must beEmpty
    }
    "bind hoursBetween" in {
      val d @ Dispatch(_, _, _) = parse("std::time::hoursBetween")
      d.binding mustEqual BuiltIn(Identifier(Vector("std", "time"), "hoursBetween"), 2, false)
      d.isReduction mustEqual false
      d.errors must beEmpty
    }
    "bind minutesBetween" in {
      val d @ Dispatch(_, _, _) = parse("std::time::minutesBetween")
      d.binding mustEqual BuiltIn(Identifier(Vector("std", "time"), "minutesBetween"), 2, false)
      d.isReduction mustEqual false
      d.errors must beEmpty
    }
    "bind secondsBetween" in {
      val d @ Dispatch(_, _, _) = parse("std::time::secondsBetween")
      d.binding mustEqual BuiltIn(Identifier(Vector("std", "time"), "secondsBetween"), 2, false)
      d.isReduction mustEqual false
      d.errors must beEmpty
    }
    "bind millisBetween" in {
      val d @ Dispatch(_, _, _) = parse("std::time::millisBetween")
      d.binding mustEqual BuiltIn(Identifier(Vector("std", "time"), "millisBetween"), 2, false)
      d.isReduction mustEqual false
      d.errors must beEmpty
    }
    "bind millisToISO" in {
