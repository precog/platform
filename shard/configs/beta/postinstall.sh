...
if status beta-shard-v1 | grep running; then
    stop beta-shard-v1
fi

sleep 5

if ! RESULT=`start beta-shard-v1 2>&1` > /dev/null ; then
...
#curl -v -f -G "http://localhost:30082/blueeyes/services/quirrel/v1/health"
