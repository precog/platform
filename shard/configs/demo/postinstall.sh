...
if status demo-shard-v1 | grep running; then
    stop demo-shard-v1
fi

sleep 5

if ! RESULT=`start demo-shard-v1 2>&1` > /dev/null ; then
...
#curl -v -f -G "http://localhost:30084/blueeyes/services/quirrel/v1/health"
