...
if status play-shard-v1 | grep running; then
    stop play-shard-v1
fi

sleep 5

if ! RESULT=`start play-shard-v1 2>&1` > /dev/null ; then
...
#curl -v -f -G "http://localhost:30080/blueeyes/services/quirrel/v1/health"
