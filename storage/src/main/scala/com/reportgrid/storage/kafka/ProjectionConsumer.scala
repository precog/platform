...
class RoutingActor(baseDir: File, routingTable: RoutingTable, metadataActor: ActorRef) extends Actor with Logging {
  ...
  def extractMetadataFor(desc: ProjectionDescriptor, boundMetadata: Set[(BoundMetadata, JValue)]): Seq[Set[Metadata]] = 
    desc.columns.map(c => boundMetadata.find(_ == c)) filter { 
      case Some(x) => true 
      case _       => false
    } map { 
      case Some(x) => x._1.metadata 
      case _       => sys.error("Theoretically unreachable code")
    }
}
...
case class ProjectionGet(idInterval : Interval[Long], sender : ActorRef)

trait ProjectionResults {
  val desc : ProjectionDescriptor
  def enumerator[A] : EnumeratorT[Unit, ByteBuffer, IO, A]
}

class ProjectionActor(projection: LevelDBProjection, descriptor: ProjectionDescriptor) extends Actor {
  implicit val bijection: Bijection[Seq[JValue], ByteBuffer] = projectionBijection2(descriptor)
  ...
  def receive = {
    ...
    case ProjectionInsert(id, values) => 
      projection.insert(id, values.as[ByteBuffer]).unsafePerformIO

    case ProjectionGet(interval, sender) =>
      sender ! new ProjectionResults {
        val desc = descriptor
        def enumerator[A] = projection.getValuesByIdRange[IO,A](interval)
      }
  }
}
