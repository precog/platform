...
trait ValueRowFormat extends RowFormat { self: StdCodecs =>
  ...
  protected def getColumnEncoder(col: (ColumnRef, Column)): Int => ByteBufferPoolS[Unit] = col match {
    ...
    case (ColumnRef(_, cType: CArrayType[_]), col: HomogeneousArrayColumn[_]) if cType == col.tpe =>
      val codec = codecForCValueType(col.tpe)
      (row: Int) => codec.write(col(row))
  ...
  protected def getColumnDecoder(col: (ColumnRef, ArrayColumn[_])): (Int, ByteBuffer) => Unit = col match {
    ...
    case (ColumnRef(_, cType: CArrayType[_]), col: ArrayHomogeneousArrayColumn[_]) if cType == col.tpe =>
      val codec = codecForCValueType(col.tpe)
      (row: Int, buf: ByteBuffer) => col.update(row, codec.read(buf))
