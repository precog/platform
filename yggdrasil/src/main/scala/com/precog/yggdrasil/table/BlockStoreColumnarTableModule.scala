  ...
  class Table(slices: StreamT[M, Slice]) extends ColumnarTable(slices) {
    ...
    def sort(sortKey: TransSpec1, sortOrder: DesiredSortOrder): M[Table] = {
      ...
      val inputOp: M[SortOutput] = tableWithSortKey.slices.foldLeft(SortOutput(Map.empty, 0, 0l)) { case (SortOutput(indices, _, nextId), slice) => {
        def columnsByPrefix(prefix: CPath, dropPrefix: Boolean): List[(ColumnRef,Column)] = slice.columns.collect {
        ...
        val dataColumns = columnsByPrefix(CPath(Value), true)
        val idColumns   = columnsByPrefix(CPath(Key), false)
        val sortColumns = columnsByPrefix(CPath(SortKey), false)
        ...
        val globalIdColumn = ColumnRef(CPath(SortGlobalId), CLong) -> new LongColumn {
